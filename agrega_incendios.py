...
#Itera sobre las llaves en pares para recorrer cada cuadrante
cuadrante_num = 1
for i in range(len(keys) - 1):
    key1 = keys[i]
    key2 = keys[i + 1]
    
    #Itera sobre los elementos de los dos identificadores en pares solapados
    for j in range(len(mi_diccionario[key1]) - 1):
        ...
        #Define los rangos de latitud y longitud para el cuadrante
        lat_min = key1
        lat_max = key2
        lon_min = elem1_key1
        lon_max = elem2_key2
        ...
        dataset_path = f'datasets_combinados/dataset_combinado_{cuadrante_num}.csv'
        dataset_completo = pd.read_csv(dataset_path)
        
        #Se convierte la fecha a un formato especial
        dataset_completo['date'] = pd.to_datetime(dataset_completo['date'])
        
        #Crea una nueva columna para almacenar los conteos de incendios
        dataset_completo['fire_count'] = 0

        #Itera sobre cada fila del dataset_completo
        for k in range(len(dataset_completo) - 1):
            #Define el rango de fechas (desde la fecha actual hasta la fecha de la siguiente fila)
            start_date = dataset_completo.loc[k, 'date']
            end_date = dataset_completo.loc[k + 1, 'date']
            
            #Filtra los incendios que caen dentro de este rango de fechas y en el cuadrante actual
            ...
            #Asignar el valor en el dataset
            dataset_completo.at[k, 'fire_count'] = conteo
        
        #Guardar el dataset completo
        output_file = f'./datasets_finales/dataset_completo_{cuadrante_num}.csv'
